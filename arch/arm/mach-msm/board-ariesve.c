...
void sec_jack_gpio_init(void)
{
	...
	rc = pm8058_gpio_config(PMIC_GPIO_MICBIAS_EN, &ear_micbias_en);
	if (rc) {
...
#if 0
	rc = pm8058_gpio_config(24, &sd_pwr_en_n); //SD_PWR_EN
	if (rc) {
		pr_err("%s PMIC_GPIO_SD_PWR_EN_N config failed\n", __func__);
		return rc;
	}
	gpio_set_value_cansleep(PM8058_GPIO_PM_TO_SYS(24), 1);
#endif	
...
static struct matrix_keymap_data ariesve_keymap_data = {
        .keymap_size    = ARRAY_SIZE(ariesve_keymap),
        .keymap         = ariesve_keymap,
};


static struct pmic8058_keypad_data ariesve_keypad_data = {
...
#if defined (CONFIG_SENSOR_MC7) || defined (CONFIG_SENSOR_CE147)
